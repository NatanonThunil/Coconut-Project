...
export default defineEventHandler(async (event) => {
    ...
    try {
        ...
        if (event.req.method === 'PUT') {
            ...
            const { id, title, title_en, description, description_en, image, status } = body;
            ...
            const query = `
                UPDATE chain_values 
                SET title = ?, title_en = ?, description = ?, description_en = ?, image = ?, status = ?
                WHERE id = ?
            `;
            const values = [title, title_en, description, description_en, imageBuffer, status ? 1 : 0, id];
            ...
            const query = 'INSERT INTO chain_values (title, title_en, description, description_en, image, status) VALUES (?, ?, ?, ?, ?, ?)';
            ...
            const [rows] = await connection.execute(
                'SELECT id, title, title_en, description, description_en, image, status FROM chain_values'
            );
