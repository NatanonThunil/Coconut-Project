    ...
    try {
        ...
        if (event.req.method === 'PUT') {
            ...
            const [rows] = await connection.execute(
                'SELECT id, title, title_en, description, description_en, category, type, image, status FROM chain_values WHERE id = ?',
                [id]
            );

            if (rows.length === 0) {
                setResponseStatus(event, 404);
                return { error: 'Chain value not found' };
            }

            const chainValue = rows[0];
            if (chainValue.image) {
                chainValue.image = `data:image/jpeg;base64,${chainValue.image.toString('base64')}`;
            }
