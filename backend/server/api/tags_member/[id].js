...
export default defineEventHandler(async (event) => {
    ...
    try {
        ...
        if (event.req.method === 'GET') {
            const [rows] = await connection.execute('SELECT * FROM tags_for_members WHERE id = ?', [id]);
            if (rows.length === 0) {
            ...
            const query = 'UPDATE tags_for_members SET text = ?, text_en = ? WHERE id = ?';
            ...
            const query = 'DELETE FROM tags_for_members WHERE id = ?';
